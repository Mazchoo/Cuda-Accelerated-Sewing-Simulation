__global__ void zero_out_normals(float* normals, const int nrNormals) {
    int vid = blockIdx.x * blockDim.x + threadIdx.x;
    if (vid >= nrNormals) return;

    float* vn = normals + vid * 3;
    vn[0] = vn[1] = vn[2] = 0.0f;
}
...
__global__ void normalize_normals(float* normals, const int nrNormals) {
    int vid = blockIdx.x * blockDim.x + threadIdx.x;
    if (vid >= nrNormals) return;
