...
def get_point_on_contour(
    ...
    if (
        start_marker > end_marker and end_marker == 0.0
    ):  # Not ideal solution but we have looped around
...
def get_closest_normal_on_mesh(
    trimesh: Trimesh, query_point: np.ndarray, distance: float = 0.0
...
def get_each_point_distance_to_3d_line(
    points: np.ndarray, vector: np.ndarray, origin: np.ndarray
) -> np.ndarray:
...
def get_closest_line_origin_for_each_point(
    points: np.ndarray, line_origins: np.ndarray, line_vector: np.ndarray
):
    """Return the index of the closest line starting at origin for each query point"""
    distances = np.array(
        [
            get_each_point_distance_to_3d_line(points, line_vector, origin)
...
def get_projections_onto_line_origins(
    points: np.ndarray, line_origins: np.ndarray, line_vector: np.ndarray
) -> np.ndarray:
...
def rotate_point_in_3d_plane(point: np.ndarray, plane: RotationPlaneData):
    ...
    rotated_current_point += (
        plane.line_vector
        * np.dot(plane.line_vector, point_vector)
        * (1 - plane.cos_theta)
    )
...
def get_bend_round_line_adjustment(
    ...
    if target_point_vector_norm == 0.0:
