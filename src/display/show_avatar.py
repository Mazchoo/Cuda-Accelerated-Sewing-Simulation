...
def show_meshes_with_annotations(
    plotly_meshes: List[go.Mesh3d],
...
def show_meshes_with_sewing_points(
    plotly_meshes: List[go.Mesh3d],
...
def show_each_mesh_different_colors(
    avatar_mesh: MeshData,
    dynamic_pieces: Dict[str, DynamicPiece],
    ...
    front_panel_plotly = create_plotly_mesh(
        front_panel_mesh, color="red", name="L-1", opacity=0.8
    )

    back_panel_mesh = dynamic_pieces["L-2"].mesh
    back_panel_plotly = create_plotly_mesh(
        back_panel_mesh, color="green", name="L-2", opacity=0.8
    )

    sleeve_right_mesh = dynamic_pieces["L-3"].mesh
    sleeve_right_plotly = create_plotly_mesh(
        sleeve_right_mesh, color="blue", name="L-3", opacity=0.8
    )

    sleeve_left_mesh = dynamic_pieces["L-3-flip"].mesh
    sleeve_left_plotly = create_plotly_mesh(
        sleeve_left_mesh, color="yellow", name="L-3-flip", opacity=0.8
    )

    return show_meshes_with_sewing_points(
        [
            avatar_plotly,
            front_panel_plotly,
            back_panel_plotly,
