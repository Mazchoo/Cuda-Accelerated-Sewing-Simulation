...
from src.utils.geometry import (
    get_closest_normal_on_mesh,
    get_each_point_distance_to_3d_line,
    get_closest_line_origin_for_each_point,
    get_projections_onto_line_origins,
    RotationPlaneData,
    get_bend_round_line_adjustment,
)

from src.parameters import WRAP_RADIANS


def get_perpedicular_alignment_along_piece(
    body_mesh: MeshData, piece: DynamicPiece, align_vector: np.ndarray
) -> Optional[np.ndarray]:
...
def get_bend_indices_on_each_side_of_alignment(
    ...
    bend_point_to_line_index = get_closest_line_origin_for_each_point(
        bend_points, line_points, vector_along_piece
    )
    bend_line_origins = line_points[bend_point_to_line_index]
    projections = get_projections_onto_line_origins(
        bend_points, bend_line_origins, vector_along_piece
    )
    ...
    postive_ind = (
        len(projections) if len(positive_proj_inds) == 0 else positive_proj_inds[0]
    )

    return (
        bend_point_to_line_index[projections_sort_inds],
        projections_sort_inds,
        postive_ind,
    )
...
def rotate_query_points_on_plane(
    origin_point: np.ndarray,
    query_inds: np.ndarray,
    vertices_3d: np.ndarray,
    rotate_calculate: RotationPlaneData,
):
...
def bend_piece_over_body(
    piece: DynamicPiece, body_mesh: MeshData, threshold: float
) -> np.ndarray:
    """
    ...
    if align_distance == 0.0:
        print(
            f"Alignment point is same location as snap point {piece.snap_point_name} -> {piece.alignment_point_name}"
        )
        return
    ...
    on_line_mask = (
        get_each_point_distance_to_3d_line(vertices_3d, align_vector, piece_snap_point)
        <= threshold
    )

    if not np.any(on_line_mask):
        print(
            f"No points near alignment line {piece.snap_point_name} -> {piece.alignment_point_name}"
        )
        return

    vector_along_piece = get_perpedicular_alignment_along_piece(
        body_mesh, piece, align_vector
    )
    ...
    (
        bend_to_line_point_inds_sorted,
        bend_projections_sort_inds,
        bend_positive_postive_ind,
    ...
    for i, origin_point in enumerate(line_points):
        query_points_mask = (
            bend_to_line_point_inds_sorted[bend_positive_postive_ind:] == i
        )
        query_inds_to_adjust = postive_sorted_query_inds[query_points_mask]
        rotate_calculate = RotationPlaneData(
            np.cos(-WRAP_RADIANS), np.sin(-WRAP_RADIANS), origin_point, align_vector
        )

        rotate_query_points_on_plane(
            origin_point, query_inds_to_adjust, vertices_3d, rotate_calculate
        )
        ...
        rotate_query_points_on_plane(
            origin_point, query_inds_to_adjust[::-1], vertices_3d, rotate_calculate
        )
