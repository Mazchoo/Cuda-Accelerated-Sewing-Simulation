...
def offset_piece_to_snap_point(
    piece: DynamicPiece, body_mesh: MeshData
) -> Optional[np.ndarray]:
    ...
    offset_target, _ = get_closest_normal_on_mesh(
        body_mesh.trimesh, body_snap_point, DISTANCE_FROM_BODY
    )
...
def rotate_point_to_alignment(
    piece: DynamicPiece, body_mesh: MeshData, snap_point: np.ndarray
) -> Optional[np.ndarray]:
    ...
    align_target, normal_to_surface = get_closest_normal_on_mesh(
        body_mesh.trimesh, body_align_point, DISTANCE_FROM_BODY
    )
    body_align_vector = align_target - snap_point
    if np.linalg.norm(body_align_vector) == 0.0:
        print(f"Alignment vector has zero distance {align_point_name}")
        return None

    piece_align_vector = piece_align_point - snap_point
    if np.linalg.norm(piece_align_vector) == 0.0:
        print(f"Piece vector has zero distance {align_point_name}")
        return None

    rotation_matrix = get_alignment_matrix(
        piece_align_vector, Z_VECTOR, body_align_vector, normal_to_surface
    )
