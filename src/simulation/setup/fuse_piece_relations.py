...
def get_piece_to_index_range_mapping(
    ...
    for piece_name, piece in pieces.items():
        piece_to_index_range[piece_name] = (
            current_offset,
...
def get_combined_particle_relations(
    pieces: Dict[str, DynamicPiece], index_ranges: PieceIndexRanges
    ...
    for piece_name, piece in pieces.items():
        ...
        all_stress_relations.append(
            get_offset_copy(piece.vertex_relations.stress_relations, start_ind)
        )
        all_shear_relations.append(
            get_offset_copy(piece.vertex_relations.shear_relations, start_ind)
        )
        all_bend_relations.append(
            get_offset_copy(piece.vertex_relations.bend_relations, start_ind)
        )

    return (
        np.concatenate(all_stress_relations),
        np.concatenate(all_shear_relations),
        np.concatenate(all_bend_relations),
    )


def get_combined_sewing_relations(
    sewing_constraints: SewingConstraints, index_ranges: PieceIndexRanges
    ...
    for sewing_pair in sewing_constraints:
        ...
        all_from_indices.append(
            get_offset_copy(sewing_pair.indices[:, 0], from_start_ind)
        )
...
def get_body_mesh_arrays(trimesh: Trimesh) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    triangles = np.array(
        [
            [trimesh.vertices[i], trimesh.vertices[j], trimesh.vertices[k]]
            for i, j, k in trimesh.faces
        ],
        dtype=np.float32,
    )
    ...
    distances_to_center = np.expand_dims(
        np.max(np.linalg.norm(triangles - np.expand_dims(centers, 1), axis=2), axis=1),
        axis=-1,
    )
