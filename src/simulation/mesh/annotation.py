...
def get_point_location(
    ...
    return get_point_on_contour(contour, tp_start, tp_end, snap_marker)
...
def get_annotation_dict_from_piece_data(piece_data: dict) -> dict:
    ...
    snap_point = get_point_location(
        piece_data["body_points"]["snap"], contour, turn_points
    )
    output[piece_data["body_points"]["snap"]["name"]] = (
        np.array([snap_point.x, snap_point.y, 0], dtype=np.float64) / CM_PER_M
    )

    alignment_point = get_point_location(
        piece_data["body_points"]["alignment"], contour, turn_points
    )
    output[piece_data["body_points"]["alignment"]["name"]] = (
        np.array([alignment_point.x, alignment_point.y, 0], dtype=np.float64) / CM_PER_M
    )
...
def get_annotated_locations_from_dict(annotated_locations: dict) -> dict:
    ...
    return {
        name: np.array(point, dtype=np.float64)
        for name, point in annotated_locations.items()
    }
