...
class SewingPairRelations:
    ...
    def __init__(
        ...
        if len(from_indices) != len(to_indices):
            raise ValueError(
                f"Lengths of sewing vertices not the same {len(from_indices)} != {len(to_indices)}"
            )

        self.indices = np.array(list(zip(from_indices, to_indices)), dtype=np.uint32)
        self.adjustment = np.zeros(
            (len(self.indices), 3), dtype=np.float64
        )  # applied in direction from to

    def recalculate_adjustment(
        self, all_from_vertices: np.ndarray, all_to_vertices: np.ndarray
    ):
        """
        ...
        adjustment_amount = (
            np.minimum(SEWING_ADJUSTMENT_STEP * TIME_DELTA, distance) / 2
        )
